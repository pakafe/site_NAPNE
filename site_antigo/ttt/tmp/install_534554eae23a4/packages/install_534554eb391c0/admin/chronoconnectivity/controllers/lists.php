						...
						if(!empty($name) AND !empty($tablename)){
							...
							if(!empty($relation)){
								...
								if(!empty($this->connection['Connection']['extras']['models']['fields'][$k])){
									$relation_settings['fields'] = array_map('trim', explode(',', $this->connection['Connection']['extras']['models']['fields'][$k]));
								}
								if(!empty($this->connection['Connection']['extras']['models']['order'][$k])){
									$relation_settings['order'] = array_map('trim', explode(',', $this->connection['Connection']['extras']['models']['order'][$k]));
								}
								if(!empty($this->connection['Connection']['extras']['models']['group'][$k])){
									$relation_settings['group'] = array_map('trim', explode(',', $this->connection['Connection']['extras']['models']['group'][$k]));
								}
		...
		if(!empty($this->connection['Connection']['extras'][$this->area]['permissions'][$this->action])){
			...
			if($this->connection['Connection']['extras'][$this->area]['permissions'][$this->action]['owner'] == 1){
				if(!empty($gcb) AND !empty($this->connection['Connection']['extras'][$this->area]['permissions_conf']['owner_id_column'])){
					$user_id = $this->connection['Connection']['extras'][$this->area]['permissions_conf']['owner_id_column'];
					$records = $this->pmodel->find('all', array('recursive' => -1, 'fields' => array($this->pmodel->pkey, $user_id), 'conditions' => array($this->pmodel->pkey => $gcb)));
					$owners = \GCore\Libs\Arr::getVal($records, array('[n]', 'Honoree', $user_id));
	...
	function index(){
		if(!empty($this->pmodel)){
			...
			if(!empty($this->connection['Connection']['extras']['models']['fields'][1])){
				$find_params['fields'] = array_map('trim', explode(',', $this->connection['Connection']['extras']['models']['fields'][1]));
			}
			if(!empty($this->connection['Connection']['extras']['models']['order'][1])){
				$this->pmodel->order_by = array_map('trim', explode(',', $this->connection['Connection']['extras']['models']['order'][1]));
			}
