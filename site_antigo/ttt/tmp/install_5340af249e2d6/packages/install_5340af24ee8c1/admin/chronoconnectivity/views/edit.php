...
<script>
	jQuery(document).ready(function($){
		...
		$('.gbs3 .tab-content > div.tab-pane:first-child').addClass('active');
...
<script>
	jQuery(document).ready(function($){
		//$('#models_tabs_heads a:last').tab('show'); //activate last tab when page loads
...
<script>
	jQuery(document).ready(function($){
		//$('#actions_tabs_heads_<?php echo $area; ?> a:last').tab('show'); //activate last tab when page loads
			...
			<div class="panel-body">
				<div class="tab-content">
					...
					<div id="models" class="tab-pane">
						...
						<button class="btn btn-success" data-g-toggle="modal" data-g-target="#add_new_model">
						...
						<div id="modelslist" class="actions_tabs" style="margin-top:10px;">
							<ul class="nav nav-tabs" id="models_tabs_heads">
								<li class="active"><a href="#modelslist-1" data-g-toggle="tab">Model1</a></li>
								<?php if(isset($this->data['Connection']['extras']['models']['name'][2])):	?>
									<?php foreach($this->data['Connection']['extras']['models']['name'] as $k => $name): ?>
										<?php if($k != 1): ?>
										<li><a href="#modelslist-<?php echo $k; ?>" data-g-toggle="tab">Model<?php echo $k; ?></a></li>
										<?php endif; ?>
									<?php endforeach; ?>
								<?php endif; ?>
							</ul>
							<div class="tab-content" id="models_tabs_contents">
								<div id="modelslist-1" class="tab-pane active">
									...
									<?php echo $this->Html->formLine('Connection[extras][models][conditions][1]', array('type' => 'textarea', 'label' => l_('CONDITIONS'), 'style' => 'width:auto;', 'rows' => 10, 'cols' => 80, 'sublabel' => 'The where conditions in array format: return array("field_name" => "XYZ");')); ?>
									<?php echo $this->Html->formLine('Connection[extras][models][fields][1]', array('type' => 'text', 'label' => l_('FIELDS'), 'class' => 'XXL', 'sublabel' => 'comma separated list of fields to be retrieved, you can leave this empty to load all fields.')); ?>
									<?php echo $this->Html->formLine('Connection[extras][models][order][1]', array('type' => 'text', 'label' => l_('ORDER'), 'class' => 'XXL', 'sublabel' => 'comma separated list of fields to be used for ordering.')); ?>
									<?php echo $this->Html->formLine('Connection[extras][models][group][1]', array('type' => 'text', 'label' => l_('GROUP'), 'class' => 'XXL', 'sublabel' => 'comma separated list of fields to be used for grouping, useful for hasMany models only.')); ?>
									<?php echo $this->Html->formSecEnd(); ?>
									<?php echo $this->Html->formEnd(); ?>
								</div>
								<?php if(isset($this->data['Connection']['extras']['models']['name'][2])):	?>
									...
									<div id="modelslist-<?php echo $k; ?>" class="tab-pane">
										...
										<?php echo $this->Html->formLine('Connection[extras][models][fields]['.$k.']', array('type' => 'text', 'label' => l_('FIELDS'), 'class' => 'XXL', 'sublabel' => 'comma separated list of fields to be retrieved from this table.')); ?>
										...
										<?php echo $this->Html->formSecEnd(); ?>
										<?php echo $this->Html->formEnd(); ?>
										<button class="remove_model_button btn btn-danger" onclick="remove_model('<?php echo $k; ?>');">
									...
									<div id="<?php echo $area; ?>-list-actions-options">
										<div class="container" style="width:100%; margin-top:20px;">
											<div class="row" style="margin-bottom:10px;">
												...
												<div class="col-md-10">
													<button class="btn btn-success" data-g-toggle="modal" data-g-target="#add_new_action_<?php echo $area; ?>">
																...
																<div class="panel panel-info">
																	...
																	<div class="panel-body">
																		<?php foreach($rules as $g_id => $g_name): ?>
																			<?php echo $this->Html->formSecStart(); ?>
																			<?php echo $this->Html->formLine('Connection[extras]['.$area.'][permissions]['.$action.']['.$g_id.']', array('type' => 'dropdown', 'label' => $g_name, 'options' => array('' => l_('NOT_SET'), 1 => l_('ALLOWED'), 0 => l_('INHERITED'), -1 => l_('DENIED')))); ?>
																			<?php echo $this->Html->formSecEnd(); ?>
																		<?php endforeach; ?>
																	</div>
																</div>
																...
																<div id="<?php echo $area; ?>-list-permissions-<?php echo $name; ?>" class="tab-pane">
																	<div class="panel panel-info">
																		...
																		<div class="panel-body">
																			<?php foreach($rules as $g_id => $g_name): ?>
																				<?php echo $this->Html->formSecStart(); ?>
																				<?php echo $this->Html->formLine('Connection[extras]['.$area.'][permissions]['.$name.']['.$g_id.']', array('type' => 'dropdown', 'label' => $g_name, 'options' => array('' => l_('NOT_SET'), 1 => l_('ALLOWED'), 0 => l_('INHERITED'), -1 => l_('DENIED')))); ?>
																				<?php echo $this->Html->formSecEnd(); ?>
																			<?php endforeach; ?>
																		</div>
																	</div>
					...
					<div id="help" class="tab-pane">
						...
						OR $this->view->Toolbar->addButton("button_id", "link_href", "TEXT", "image_path", "submit_selectors");
						</p>
						<p>
...
<div id="model_generic_config" class="generic_action_config" title="Generic" style="display:none;">
	...
	<?php echo $this->Html->formSecEnd(); ?>
	<?php echo $this->Html->formEnd(); ?>
	<button class="remove_model_button btn btn-danger" onclick="remove_model('{N}');">
		<?php echo l_('REMOVE_MODEL'); ?>
	</button>
</div>
