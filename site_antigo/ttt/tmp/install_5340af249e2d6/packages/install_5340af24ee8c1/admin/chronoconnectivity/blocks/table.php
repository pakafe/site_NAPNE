...
class Table {
	...
	public static function config($area = 'front'){
		...
		echo \GCore\Helpers\Html::formLine('Connection[extras]['.$area.'][blocks][table][header]', array('type' => 'textarea', 'label' => l_('HEADER_CODE'), 'style' => 'width:auto;', 'rows' => 18, 'cols' => 80, 'sublabel' => 'HTML/PHP code for the header of the list, this code is rendered only once.'));
		echo \GCore\Helpers\Html::formLine('Connection[extras]['.$area.'][blocks][table][footer]', array('type' => 'textarea', 'label' => l_('FOOTER_CODE'), 'style' => 'width:auto;', 'rows' => 18, 'cols' => 80, 'sublabel' => 'HTML/PHP code for the footer of the list, this code is rendered only once.'));
	...
	public function display($connection, $area = 'front', $rows = array()){
		...
		//echo '<div class="gcore chrono-page-container">';
		//display header
		ob_start();
