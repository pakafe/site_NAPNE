...
class Lister {
	...
	public function prepare($connection, $string = ''){
		if(strpos($string, '_TOOLBAR_NEW_') !== false){
			$string = str_replace('_TOOLBAR_NEW_', '', $string);
			echo $this->view->Toolbar->renderButton('add', r_('index.php?ext=chronoconnectivity&cont=lists&act=edit&ccname='.$connection['Connection']['title']), l_('_NEW_'), $this->view->Assets->image('add', 'toolbar/'));
		}
		if(strpos($string, '_TOOLBAR_DELETE_') !== false){
			$string = str_replace('_TOOLBAR_DELETE_', '', $string);
			echo $this->view->Toolbar->renderButton('remove', r_('index.php?ext=chronoconnectivity&cont=lists&act=delete&ccname='.$connection['Connection']['title']), l_('DELETE'), $this->view->Assets->image('remove', 'toolbar/'), 'submit_selectors');
		}
		if(strpos($string, '_TOOLBAR_SAVELIST_') !== false){
			$string = str_replace('_TOOLBAR_SAVELIST_', '', $string);
			echo $this->view->Toolbar->renderButton('save_list', r_('index.php?ext=chronoconnectivity&cont=lists&act=save_list&ccname='.$connection['Connection']['title']), l_('UPDATE_LIST'), $this->view->Assets->image('save_list', 'toolbar/'), 'submit_selectors');
		}

		if(strpos($string, '_TOOLBAR_SAVE_') !== false){
			$string = str_replace('_TOOLBAR_SAVE_', '', $string);
			echo $this->view->Toolbar->renderButton('save', r_('index.php?ext=chronoconnectivity&cont=lists&act=save&ccname='.$connection['Connection']['title']), l_('SAVE'), $this->view->Assets->image('save', 'toolbar/'));
		}
		if(strpos($string, '_TOOLBAR_APPLY_') !== false){
			$string = str_replace('_TOOLBAR_APPLY_', '', $string);
			echo $this->view->Toolbar->renderButton('apply', r_('index.php?ext=chronoconnectivity&cont=lists&act=save&ccname='.$connection['Connection']['title'].'&save_act=apply'), l_('APPLY'), $this->view->Assets->image('apply', 'toolbar/'));
		}
		if(strpos($string, '_TOOLBAR_CANCEL_') !== false){
			$string = str_replace('_TOOLBAR_CANCEL_', '', $string);
			echo $this->view->Toolbar->renderButton('cancel', r_('index.php?ext=chronoconnectivity&cont=lists&act=index&ccname='.$connection['Connection']['title']), l_('CANCEL'), $this->view->Assets->image('cancel', 'toolbar/'), 'link');
		}
